...
- `/account` — listings where `owner_id` is the current user, with time left from `endsAt` and closed after end. Live listings have “Remove listing”. Confirm: “This refunds all live deposits and takes the listing down.” `POST /api/listings/[id]/remove` requires the session `owner_id`. If the signed-in email is in `ADMIN_EMAILS` (comma-separated, server-only), `/account` also lists every live listing with “Admin remove” (same API, `{ admin: true }`). Shows Connect status: not started / pending / ready. Requires login.
