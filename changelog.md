...
- No platform fee on the 20% deposit. Deposits stay on the platform for refunds. After `ends_at`, admin Close auction or hourly cron closes the listing: live high bid on each spot wins; remaining 80% Payment Link charges `application_fee_amount` = 10% of the winning bid; outbid bids are not charged. Test keys only.
