...
- No platform fee on the 20% deposit. Deposits stay on the platform. Admin (`ADMIN_EMAILS`) can Close auction on any live listing now (also on their own `/account` rows): live high bids win; remaining 80% Payment Link takes 10% of each winning bid; deposit stays (no extra fee). Owner Close early only if there are zero live bids. When `ends_at` passes, the listing/account/checkout path or daily cron closes and bills winners the remaining 80%. Get a spot has an optional Message for the lister. Site nav and `/account` show the signed-in email as the account name. Test keys only.
