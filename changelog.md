...
- No platform fee on the 20% deposit. Deposits stay on the platform. Admin (`ADMIN_EMAILS`) can Close auction on any live listing now: live high bids win; remaining 80% Payment Link takes 10% of each winning bid; deposit stays (no extra fee). Owner Close early only if there are zero live bids. When `ends_at` passes, hourly cron uses the same winner + Payment Link path. Test keys only.
